...
function handleIncludes (resp, includes, relations, options) {
  ...
  if (!_.isArray(includes) && typeof includes !== 'object') {
    throw RelUtils.getInvalidIncludesError(
      'JSON API unable to detect valid includes'
    )
  }

  var embedded = resources.map(function substituteEmbeddedForIds (resource) {
    if (!Array.isArray(includes)) {
      const include = Object.keys(includes)[0]
      return fillIncluded(app, include, relations, resource, options)
    }
    return includes.map(include => {
      if (typeof include === 'object') {
        const incl = Object.keys(include)[0]
        return fillIncluded(app, incl, relations, resource, options)
      }
      return fillIncluded(app, include, relations, resource, options)
    })
  })
  ...
  if (embedded.length) {
    ...
    if (typeof includes === 'string') {
      ...
      const isDeepIncludesExists = includes.find(i => typeof i !== 'string')
      if (!isDeepIncludesExists) {
        included = unique
      } else {
        includes.forEach(incl => {
          if (typeof incl === 'object') {
            const [prop, nestedProp] = Object.entries(incl)[0]
            included = getNestedIncludes(unique, prop, nestedProp)
          }
        })
      }
    } else {
      const [prop, nestedProp] = Object.entries(includes)[0]
      included = getNestedIncludes(unique, prop, nestedProp)
    }
...
function getNestedIncludes (included, prop, nestedName) {
  const isNestedExists = typeof nestedName === 'object'
  const nestedProp = !isNestedExists ? nestedName : Object.keys(nestedName)[0]

  const newIncluded = included.reduce(
    (acc, item) => {
      if (
        prop.indexOf(item.type) !== -1 &&
        (_.has(item.attributes, nestedProp) || _.has(item, nestedProp))
      ) {
        let nestedItems

        if (_.has(item.attributes, nestedProp)) {
          nestedItems = item.attributes[nestedProp].map(nestedItem => {
            nestedItem.type = nestedProp
            return createNewIncludeItem(nestedItem)
          })
          delete item.attributes[nestedProp]
        } else {
          nestedItems = item[nestedProp].map(nestedItem => {
            nestedItem.type = nestedProp
            return createNewIncludeItem(nestedItem)
          })
        ...
        item.relationships[nestedProp].data = nestedItems.map(nestedItem => {
          return {
            id: nestedItem.id,
  ...
  if (isNestedExists) {
    return getNestedIncludes(newIncluded, nestedProp, nestedName[nestedProp])
  }
  return newIncluded
}

function createNewIncludeItem (data) {
  return {
    id: data.id,
    type: data.type,
...
function fillIncluded (app, include, relations, resource, options) {
